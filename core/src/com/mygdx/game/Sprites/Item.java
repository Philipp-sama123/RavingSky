...
public class Item {
    ...
    public void spawnItem() {
        if (itemCount < MyGdxGame.COIN_SPAWN_FREQUENCY) {
            itemCount++;
        } else {
            itemCount = 0;
            makeItem();
    ...
    private void setItemVelocity(boolean isMainCharacterAwesome, int i) {
        ...
        if (isMainCharacterAwesome) {
            if (isGoodItem) {
                itemXs.set(i, itemXs.get(i) - MyGdxGame.COIN_VELOCITY_IN_PX * MyGdxGame.AWESOME_VELOCITY_MULTIPLIER_IN_PX);
            } else {
                itemXs.set(i, itemXs.get(i) - MyGdxGame.BOMB_VELOCITY_IN_PX * MyGdxGame.AWESOME_VELOCITY_MULTIPLIER_IN_PX);
            }
        } else {
            if (isGoodItem) {
                itemXs.set(i, itemXs.get(i) - MyGdxGame.COIN_VELOCITY_IN_PX);
            } else {
                itemXs.set(i, itemXs.get(i) - MyGdxGame.BOMB_VELOCITY_IN_PX);
            }
        }
    }
